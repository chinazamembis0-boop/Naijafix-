-- NaijaFix: Atomic booking acceptance RPC
-- Fixes the race condition where two providers could accept the same
-- Pending booking simultaneously. The final database operation guarantees
-- that only one provider can successfully accept a booking.
--
-- Safe, idempotent migration. Additive only. No existing tables, columns,
-- policies, or functions are dropped, renamed, or repurposed.

-- ============================================================
-- A. accept_booking(p_booking_id) — atomic Pending -> Accepted
-- ============================================================
-- Security:
--   - SECURITY DEFINER so RLS cannot block the transition.
--   - SET search_path = '' and every reference is schema-qualified.
--   - Validates the caller is a provider in the providers table.
--   - Validates the booking exists, is currently Pending, and is assigned
--     to the caller's provider_user_id.
--   - Uses SELECT ... FOR UPDATE SKIP LOCKED to serialize concurrent
--     accept attempts on the same row. The first transaction to lock the
--     row proceeds; later ones skip it and see the updated status.
--   - Returns a JSON result with an explicit 'accepted' boolean so the
--     caller can distinguish a successful accept from a no-op.
--   - Sends a notification ONLY when the transition actually occurred.
