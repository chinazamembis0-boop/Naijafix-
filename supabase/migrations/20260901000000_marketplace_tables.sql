-- NaijaFix: Missing marketplace tables, columns, indexes, RLS policies, and grants
-- Safe idempotent migration for objects that were defined in
-- marketplace_extensions.sql but never applied to the remote database.
-- Run in Supabase SQL Editor.
-- Does NOT modify existing data, tables, columns, or policies.
...
begin
  ...
  if not exists (
    ...
    create policy "Customers can accept or decline quotes"
      on public.quotes for update to authenticated
      using (exists (select 1 from public.bookings b where b.id = booking_id and b.customer_user_id = auth.uid()))
      with check (exists (select 1 from public.bookings b where b.id = booking_id and b.customer_user_id = auth.uid()));
  end if;
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'booking_photos'
...
-- ============================================================
-- I. Storage policies for booking-photos (corrected)
-- ============================================================
do $$
begin
  -- Corrected INSERT: checks folder = auth.uid() instead of requiring a booking_photos row
  if not exists (
    select 1 from pg_policies
  ...
  -- SELECT: restricted to booking participants via booking_photos + bookings tables
  if not exists (
    select 1 from pg_policies
    where schemaname = 'storage' and tablename = 'objects'
      and policyname = 'Booking participants can view booking photos'
