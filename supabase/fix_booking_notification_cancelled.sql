-- Add cancelled event to booking notification helper
-- Run in Supabase SQL Editor after create_booking_notification.sql.
...
begin
  ...
  elsif p_event in ('accepted', 'declined', 'pending', 'provider on the way', 'in progress', 'completed', 'cancelled') and v_customer_user_id is not null then
    insert into public.notifications (user_id, type, title, message, is_read, created_at)
    values (
      v_customer_user_id,
      'booking',
